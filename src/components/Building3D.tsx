...
export default function Building3D({
  ...
  const getApartmentColor = useCallback(
    (
      ...
      if (apartment.documents?.length > 0)
        return apartment.documents[0].color || "#fff";
      return "#fff";
    },
    [selectedApartments],
  );
    ...
    <>
      ...
      <group ref={groupRef}>
        {building.sections.map((section: any, sectionIndex: number) => {
          ...
          const apartments = section.apartments.slice(
            0,
            section.apartmentsCount,
          );
          const segmentWidth = (2 * Math.PI) / apartments.length;
          const radialSegments =
            apartments.length >= 4
              ? 1
              : Math.max(1, Math.floor(60 / apartments.length));
              ...
              {Array.from({ length: floorCount }, (_, floorIndex) => {
                ...
                return (
                  <group key={floorIndex} position={[0, yPos, 0]}>
                    {apartments.map(
                      (apartment: any, apartmentIndex: number) => (
                        <ApartmentSlice
                          key={apartmentIndex}
