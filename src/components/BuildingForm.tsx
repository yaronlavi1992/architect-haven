...
// In edit mode: always show individual floor rows (no grouped ranges)
const sectionsToIndividualRows = (sections: any[]): FloorGroup[] => {
  if (!sections?.length) return [];
  const sorted = [...sections].sort((a, b) => a.startFloor - b.startFloor);
  const rows: FloorGroup[] = [];
  for (const s of sorted) {
    for (let floor = s.startFloor; floor <= s.endFloor; floor++) {
      rows.push({
        startFloor: floor,
        endFloor: floor,
        apartmentsCount: s.apartmentsCount ?? s.apartments?.length ?? 8,
        description: s.description ?? "offices",
      });
    }
  }
  return rows;
...
export default function BuildingForm({
  ...
  useEffect(() => {
    if (building?.sections?.length) {
      ...
      const rows = sectionsToIndividualRows(building.sections);
      setSections(rows);
    } else if (building) {
      setName(building.name);
      setSections([
  ...
  const addSection = () => {
    ...
    setSections([
      ...sections,
      {
        ...
        endFloor: isEdit ? newStartFloor : newStartFloor + 1,
        apartmentsCount: 8,
        description: "offices",
      },
    ]);
  ...
  const updateSection = (
    ...
    if (building && (field === "startFloor" || field === "endFloor")) {
      const floor =
        typeof value === "number" ? value : parseInt(String(value), 10) || 1;
      updated[index].startFloor = updated[index].endFloor = Math.max(1, floor);
    }
    setSections(updated);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Expand ranges into individual floors for storage (original logic)
      const expandedSections: any[] = [];
      const existingSections = building?.sections;

      sections.forEach((section) => {
        for (
          let floor = section.startFloor;
          floor <= section.endFloor;
          floor++
        ) {
          const existingSection = existingSections?.find(
            (s: any) => floor >= s.startFloor && floor <= s.endFloor,
          );
      ...
      if (msg.includes("Free plan limited") || msg.includes("Upgrade to Pro")) {
        ...
        toast.error(
          `Free plan limit reached (${limit} buildings). Upgrade to Pro for unlimited.`,
        );
      } else {
        toast.error(msg);
      }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-xl">
